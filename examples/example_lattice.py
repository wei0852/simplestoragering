...
def generate_ring() -> ssr.CSLattice:
    ...
    Q1 = ssr.Quadrupole('Q1', length=0.220000, k1=5.818824, n_slices=10)
    Q2 = ssr.Quadrupole('Q2', length=0.220000, k1=-6.088408, n_slices=10)
...
#     O1 = ssr.Octupole('O1', length=1e-4, k3=0, n_slices=4) 
    ...
    CELLH = [ss, D1A, SF1, D1B, Q1, O1, D2A, SD1, D2B, Q2, D3, ss, B1, D4, SD2, D5, Q3,
...
def generate_ring_6D() -> ssr.CSLattice:
    d2r = np.pi / 180
    ssr.set_ref_energy(2200)
    D1 = ssr.Drift('D1', length=2.650000)
    D1A = ssr.Drift('D1A', length=2.475000)
...
#     O1 = ssr.Octupole('O1', length=1e-4, k3=0, n_slices=4) 
    ...
    rf = ssr.RFCavity('RFCavity', 0.92, HarmNumber * 299792458 / L0, int(HarmNumber), 0.0)
    RING = ssr.CSLattice([ssr.Mark('INJ')] + (CELLH + [B2] + RC) * 14 + [rf])
    RING.linear_optics()
    return RING
